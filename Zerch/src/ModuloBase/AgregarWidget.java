...
public class AgregarWidget extends JFrame implements ActionListener
{
	...
	// para saber a cual ranura de edicion pertenece el widget clickeado (talves
	// innecesario con recursion actual)
	Ranura ranura_actual;

	// el widget sobre el cual se agregara el widget (contenedor=ventana
	// interna, plantilla, panel)
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		{
			string_hoja = ""
					+ arbol_swing.arbol_total.getLastSelectedPathComponent();

			int location_x = Integer.parseInt(
					panel_propiedad.textfield_posx.getText().toString());
			int location_y = Integer.parseInt(
					panel_propiedad.textfield_posy.getText().toString());
			int size_w = Integer.parseInt(
					panel_propiedad.textfield_size_w.getText().toString());
			int size_h = Integer.parseInt(
					panel_propiedad.textfield_size_h.getText().toString());
			...
			{
				funcion_base.notificarPanel(location_x, location_y, size_w,
						size_h, pos);
			}

			if (string_hoja.endsWith("SButton"))
			{
				funcion_base.notificarButton(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SRadioButton"))
			{
				funcion_base.notificarRadioButton(location_x, location_y,
						size_w, size_h, pos, texto);
			}

			if (string_hoja.endsWith("SCheckBox"))
			{
				funcion_base.notificarCheckBox(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("STextField"))
			{
				funcion_base.notificarTextField(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SPasswordField"))
			{
				funcion_base.notificarPasswordField(location_x, location_y,
						size_w, size_h, pos, texto);
			}

			if (string_hoja.endsWith("STextArea"))
			{
				funcion_base.notificarTextArea(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SList"))
			{
				funcion_base.notificarList(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SComboBox"))
			{
				funcion_base.notificarComboBox(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SSlider"))
			{
				funcion_base.notificarSlider(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SLabel"))
			{
				funcion_base.notificarLabel(location_x, location_y, size_w,
						size_h, pos, texto);
			}

			if (string_hoja.endsWith("SProgressBar"))
			{
				funcion_base.notificarProgresBar(location_x, location_y, size_w,
						size_h, pos, texto);
			}
			funcion_base.ventana_base.setSize(
					funcion_base.ventana_base.getWidth() + 1,
					funcion_base.ventana_base.getHeight() + 1);
			this.dispose();
		}
	}
}